...
void emitDriverStatsRemark(
    ...
    const mlir::egraph::EGraphRewriteDriverResult &result) {
  std::string buffer;
  llvm::raw_string_ostream os(buffer);
  mlir::egraph::printEGraphRewriteDriverResult(os, result);
  os.flush();
...
mlir::LogicalResult
verifyExtractRequestRoots(mlir::Operation *anchor,
                          const mlir::egraph::EGraphExtractRequest &request,
                          llvm::ArrayRef<mlir::StringAttr> expectedRoots) {
  if (request.roots.size() != expectedRoots.size())
    return anchor->emitOpError("extract request returned ")
           << request.roots.size() << " root(s), expected "
           << expectedRoots.size();

  for (auto indexedRoot : llvm::enumerate(request.roots)) {
    mlir::egraph::EValue root = indexedRoot.value();
    if (root.getResultIndex() != 0)
      return anchor->emitOpError("extract request root #")
...
mlir::LogicalResult verifyExtractRequestSemantics(mlir::ModuleOp module,
                                                  mlir::egraph::EGraph &graph) {
  mlir::FailureOr<mlir::egraph::EGraphOp> egraph =
      getSingleActiveEGraph(module);
  if (mlir::failed(egraph))
  ...
  mlir::FailureOr<mlir::egraph::EGraphExtractRequest> defaultRequest =
      mlir::egraph::buildEGraphExtractRequest(graph, *egraph);
  if (mlir::failed(defaultRequest))
    return egraph->emitOpError("failed to build default extract request");
  if (defaultRequest->mode != mlir::egraph::EGraphExtractMode::Greedy)
    return egraph->emitOpError("default extract request used a wrong mode");
  llvm::SmallVector<mlir::StringAttr, 4> defaultRoots = {rhs.getSymNameAttr(),
                                                         user.getSymNameAttr()};
  if (mlir::failed(verifyExtractRequestRoots(egraph->getOperation(),
                                             *defaultRequest, defaultRoots)))
    return mlir::failure();

  egraph->emitRemark() << "extract request default roots -> "
                       << formatExtractRoots(defaultRequest->roots) << " mode="
  ...
  mlir::FailureOr<mlir::egraph::EGraphExtractRequest> explicitRequest =
      mlir::egraph::buildEGraphExtractRequest(
          graph, *egraph, explicitRoots,
          mlir::egraph::EGraphExtractMode::LinearProgramming);
  if (mlir::failed(explicitRequest))
    return egraph->emitOpError("failed to build explicit extract request");
  if (explicitRequest->mode !=
      mlir::egraph::EGraphExtractMode::LinearProgramming)
    return egraph->emitOpError("explicit extract request lost its mode");
  llvm::SmallVector<mlir::StringAttr, 4> explicitExpectedRoots = {
      lhs.getSymNameAttr(), user.getSymNameAttr()};
  if (mlir::failed(verifyExtractRequestRoots(
          egraph->getOperation(), *explicitRequest, explicitExpectedRoots)))
    return mlir::failure();

  egraph->emitRemark() << "extract request explicit roots -> "
                       << formatExtractRoots(explicitRequest->roots) << " mode="
                       << mlir::egraph::stringifyEGraphExtractMode(
                              explicitRequest->mode);

  mlir::FailureOr<mlir::egraph::EGraphUnionResult> unioned =
      graph.unionValues(graph.getValue(lhs.getSymNameAttr()),
                        graph.getValue(rhs.getSymNameAttr()));
  if (mlir::failed(unioned) || !unioned->changed)
    return egraph->emitOpError("failed to dirty extract request graph");
  if (graph.isClean())
    return egraph->emitOpError("dirty union unexpectedly left graph clean");
  if (mlir::succeeded(mlir::egraph::buildEGraphExtractRequest(graph, *egraph)))
    return egraph->emitOpError(
        "extract request unexpectedly accepted a dirty graph");

  egraph->emitRemark() << "extract request rejected dirty graph";
  return mlir::success();
}
...
verifyExtractCostModelSelection(mlir::ModuleOp module,
  ...
  if (mlir::failed(request))
    return egraph->emitOpError("failed to build extract request");

  auto costModel = [](mlir::egraph::EOpRefBase candidate)
      -> mlir::FailureOr<mlir::egraph::EGraphExtractCost> {
    if (candidate.getOperationName() == "arith.muli")
      return mlir::egraph::EGraphExtractCost(1);
    if (candidate.getOperationName() == "arith.addi")
      return mlir::egraph::EGraphExtractCost(7);
    return mlir::failure();
  };

  mlir::FailureOr<mlir::egraph::EGraphExtractResult> selection =
      mlir::egraph::selectEGraphExtractCandidates(graph, *request, costModel);
...
findExtractSelection(const mlir::egraph::EGraphExtractResult &selection,
...
mlir::LogicalResult verifyExtractAliasSelection(
    mlir::Operation *anchor, const mlir::egraph::EGraphExtractResult &selection,
...
mlir::LogicalResult verifyGreedyExtract(mlir::ModuleOp module,
  ...
  if (egraph->getSymName() == "rebuilt_input_alias") {
    ...
    if (mlir::failed(request))
      return egraph->emitOpError(
          "failed to build rebuilt input alias extract request");
    ...
    mlir::FailureOr<mlir::egraph::EGraphExtractResult> selection =
        mlir::egraph::extractEGraphGreedily(graph, *egraph, *request,
                                            costModel);
    if (mlir::failed(selection))
      return egraph->emitOpError(
  ...
  mlir::FailureOr<mlir::egraph::EGraphExtractRequest> request =
      mlir::egraph::buildEGraphExtractRequest(graph, *egraph);
  if (mlir::failed(request))
    return egraph->emitOpError("failed to build greedy extract request");
  ...
  if (mlir::failed(verifyExtractRequestRoots(egraph->getOperation(), *request,
                                             expectedRoots)))
    return mlir::failure();
  ...
  auto costModel = [](mlir::egraph::EOpRefBase candidate)
    ...
    if (candidate.getOperationName() == "arith.subi")
      return mlir::egraph::EGraphExtractCost(10);
    return mlir::failure();
  };

  mlir::FailureOr<mlir::egraph::EGraphExtractResult> selection =
      mlir::egraph::extractEGraphGreedily(graph, *egraph, *request, costModel);
  if (mlir::failed(selection))
  ...
  mlir::FailureOr<mlir::egraph::EGraphExtractRequest> cyclicRequest =
      mlir::egraph::buildEGraphExtractRequest(graph, *egraph, cyclicRoots);
  if (mlir::failed(cyclicRequest))
    return egraph->emitOpError("failed to build cyclic greedy request");

  if (mlir::succeeded(mlir::egraph::extractEGraphGreedily(
          graph, *egraph, *cyclicRequest, costModel)))
...
verifyLinearProgrammingExtract(mlir::ModuleOp module,
  ...
  mlir::FailureOr<mlir::egraph::EGraphExtractRequest> request =
      mlir::egraph::buildEGraphExtractRequest(
          ...
          mlir::egraph::EGraphExtractMode::LinearProgramming);
  if (mlir::failed(request))
    return egraph->emitOpError("failed to build LP extract request");
  ...
  auto costModel = [](mlir::egraph::EOpRefBase candidate)
      -> mlir::FailureOr<mlir::egraph::EGraphExtractCost> {
    llvm::StringRef owner = candidate.getEClassOp().getSymName();
  ...
  mlir::FailureOr<mlir::egraph::EGraphExtractResult> selection =
      mlir::egraph::extractEGraphByLinearProgramming(graph, *egraph, *request,
                                                     costModel);
  if (mlir::failed(selection))
  ...
  mlir::FailureOr<mlir::egraph::EGraphExtractRequest> cyclicRequest =
      mlir::egraph::buildEGraphExtractRequest(
          graph, *egraph, cyclicRoots,
          mlir::egraph::EGraphExtractMode::LinearProgramming);
  if (mlir::failed(cyclicRequest))
    return egraph->emitOpError("failed to build cyclic LP request");

  if (mlir::succeeded(mlir::egraph::extractEGraphByLinearProgramming(
          graph, *egraph, *cyclicRequest, costModel)))
...
struct RefWithStructuralKey {
  mlir::egraph::EOpRefBase ref;
  mlir::egraph::EGraphStructuralKey key;
};

mlir::LogicalResult
collectOpAStructuralKeys(mlir::egraph::EGraph &graph,
                         llvm::SmallVectorImpl<RefWithStructuralKey> &refs) {
  for (mlir::egraph::EOpRefBase ref : graph.getOpRefs()) {
    if (!llvm::isa<mlir::egraph::test::OpAOp>(ref.getOperation()))
      continue;

    mlir::FailureOr<mlir::egraph::EGraphStructuralKey> key =
        graph.getStructuralKey(ref);
    if (mlir::failed(key))
      return ref.getOperation()->emitOpError("structural key lookup failed");

    refs.push_back({ref, std::move(*key)});
  }
...
mlir::LogicalResult
verifyDuplicateStructuralKey(mlir::egraph::EGraph &graph,
                             llvm::ArrayRef<RefWithStructuralKey> refs,
                             RefWithStructuralKey &firstDuplicate,
                             RefWithStructuralKey &secondDuplicate) {
  ...
  for (unsigned i = 0, e = refs.size(); i < e; ++i) {
    for (unsigned j = i + 1; j < e; ++j) {
      ...
      if (foundDuplicate)
        return refs[j].ref.getOperation()->emitOpError(
            "expected exactly one duplicate structural key pair");

      firstDuplicate = refs[i];
      secondDuplicate = refs[j];
      foundDuplicate = true;
    }
  }
  ...
  if (!foundDuplicate) {
    if (refs.empty())
      return mlir::failure();
    return refs.front().ref.getOperation()->emitOpError(
        "expected duplicate structural key pair");
  }

  mlir::FailureOr<mlir::egraph::EGraphHashConsEntry> duplicateEntry =
      graph.lookupStructuralEntry(secondDuplicate.ref);
  if (mlir::failed(duplicateEntry))
    return secondDuplicate.ref.getOperation()->emitOpError(
        "duplicate structural entry lookup failed");

  if (duplicateEntry->operation != firstDuplicate.ref.getOperation())
    return secondDuplicate.ref.getOperation()->emitOpError(
        "duplicate structural key did not reuse the first occurrence");

  if (duplicateEntry->resultKeyValues.empty())
    return secondDuplicate.ref.getOperation()->emitOpError(
        "duplicate structural key did not reuse the first eclass result");

  mlir::FailureOr<mlir::egraph::EValue> duplicateResult =
      graph.lookupValue(duplicateEntry->resultKeyValues.front());
  if (mlir::failed(duplicateResult) ||
      *duplicateResult != firstDuplicate.ref.getResult(0).getLeader())
    return secondDuplicate.ref.getOperation()->emitOpError(
        "duplicate structural key did not reuse the first eclass result");

  secondDuplicate.ref.getOperation()->emitRemark()
      << "structural hashcons reused existing eclass";
  return mlir::success();
}

mlir::LogicalResult verifyDistinctStructuralKey(
    mlir::egraph::EGraph &graph, const RefWithStructuralKey &duplicate,
    const RefWithStructuralKey &candidate, llvm::StringRef remark) {
  if (candidate.key == duplicate.key)
    return candidate.ref.getOperation()->emitOpError(
        "expected distinct structural key");

  mlir::FailureOr<mlir::egraph::EGraphHashConsEntry> entry =
      graph.lookupStructuralEntry(candidate.ref);
  if (mlir::failed(entry))
    return candidate.ref.getOperation()->emitOpError(
        "distinct structural entry lookup failed");

  if (entry->operation != candidate.ref.getOperation())
    return candidate.ref.getOperation()->emitOpError(
        "distinct structural key reused an unrelated occurrence");

  candidate.ref.getOperation()->emitRemark() << remark;
  return mlir::success();
}

mlir::LogicalResult
verifyMultiResultStructuralKey(mlir::egraph::EGraph &graph) {
  for (mlir::egraph::EOpRefBase ref : graph.getOpRefs()) {
    if (!llvm::isa<mlir::egraph::test::SplitOp>(ref.getOperation()))
      continue;
    ...
    mlir::FailureOr<mlir::egraph::EGraphHashConsEntry> entry =
        graph.lookupStructuralEntry(ref);
    if (mlir::failed(entry))
      return ref.getOperation()->emitOpError(
          "multi-result structural entry lookup failed");

    if (key->resultTypes.size() != 2 || key->childKeyValues.size() != 1 ||
        entry->resultKeyValues.size() != 2)
      return ref.getOperation()->emitOpError(
          "multi-result structural key lost tuple shape");

    mlir::FailureOr<mlir::egraph::EValue> result0 =
        graph.lookupValue(entry->resultKeyValues[0]);
    mlir::FailureOr<mlir::egraph::EValue> result1 =
        graph.lookupValue(entry->resultKeyValues[1]);
    if (mlir::failed(result0) || mlir::failed(result1) ||
        *result0 != ref.getResult(0).getLeader() ||
        *result1 != ref.getResult(1).getLeader())
      return ref.getOperation()->emitOpError(
          "multi-result structural key lost tuple shape");

    ref.getOperation()->emitRemark()
        << "multi-result structural key recorded result tuple";
    return mlir::success();
  }
...
mlir::LogicalResult verifyStructuralKeyHashCons(mlir::egraph::EGraph &graph) {
  llvm::SmallVector<RefWithStructuralKey> refs;
  if (mlir::failed(collectOpAStructuralKeys(graph, refs)))
    return mlir::failure();

  RefWithStructuralKey firstDuplicate;
  RefWithStructuralKey secondDuplicate;
  if (mlir::failed(verifyDuplicateStructuralKey(graph, refs, firstDuplicate,
                                                secondDuplicate)))
    return mlir::failure();

  bool checkedDifferentAttributes = false;
  bool checkedDifferentChildren = false;
  for (const RefWithStructuralKey &candidate : refs) {
    if (candidate.ref == firstDuplicate.ref ||
        candidate.ref == secondDuplicate.ref)
      continue;

    if (getOpAMode(candidate.ref) == "different_attr") {
      if (mlir::failed(verifyDistinctStructuralKey(
              graph, firstDuplicate, candidate,
              "structural key kept different attributes separate")))
        return mlir::failure();
      checkedDifferentAttributes = true;
      continue;
    }

    if (getOpAMode(candidate.ref) == "same") {
      if (mlir::failed(verifyDistinctStructuralKey(
              graph, firstDuplicate, candidate,
              "structural key kept different children separate")))
        return mlir::failure();
      checkedDifferentChildren = true;
    }
  }

  if (!checkedDifferentAttributes)
    return firstDuplicate.ref.getOperation()->emitOpError(
        "missing different-attribute structural key case");
  if (!checkedDifferentChildren)
    return firstDuplicate.ref.getOperation()->emitOpError(
        "missing different-child structural key case");

  return verifyMultiResultStructuralKey(graph);
}
...
llvm::SmallVector<mlir::egraph::EOpRefBase>
findSplitRefs(mlir::egraph::EGraph &graph) {
  llvm::SmallVector<mlir::egraph::EOpRefBase> refs;
  for (mlir::egraph::EOpRefBase ref : graph.getOpRefs())
    if (llvm::isa<mlir::egraph::test::SplitOp>(ref.getOperation()))
      refs.push_back(ref);
...
mlir::LogicalResult
verifyReachableScratchReplacement(mlir::ModuleOp module,
                                  mlir::egraph::EGraph &graph,
                                  mlir::egraph::EOpRefBase opRef) {
  mlir::egraph::EGraphRewriteTransaction transaction(graph,
                                                     module.getContext());
  ...
  mlir::Location loc = opRef.getLoc();
  mlir::Type resultType = opRef.getOperation()->getResult(0).getType();
  auto unused = mlir::egraph::test::LeafOp::create(
      rewriter, loc, resultType, rewriter.getStringAttr("unused"));
  auto leaf = mlir::egraph::test::LeafOp::create(
      rewriter, loc, resultType, rewriter.getStringAttr("reachable-leaf"));
  auto mid = mlir::egraph::test::OpBOp::create(
      rewriter, loc, resultType, leaf.getResult(),
      rewriter.getStringAttr("reachable-mid"));
  auto root = mlir::egraph::test::OpAOp::create(
      rewriter, loc, resultType, mid.getResult(),
      opRef.getOperation()->getOperand(1),
      rewriter.getStringAttr("reachable-root"));

  if (mlir::failed(rewriter.replaceOp(opRef, root->getResults())))
    return opRef.getOperation()->emitOpError(
        "failed to record scratch DAG replacement");

  mlir::FailureOr<llvm::SmallVector<mlir::Operation *>> reachable =
      transaction.collectReachableScratchOperations();
  if (mlir::failed(reachable))
    return opRef.getOperation()->emitOpError(
        "failed to collect reachable scratch operations");

  llvm::ArrayRef<mlir::Operation *> reachableOps = *reachable;
  if (reachableOps.size() != 3)
    return opRef.getOperation()->emitOpError(
        "expected three reachable scratch operations");
  if (llvm::is_contained(reachableOps, unused.getOperation()))
    return unused.emitOpError("unused scratch operation was collected");
  if (reachableOps[0] != leaf.getOperation() ||
      reachableOps[1] != mid.getOperation() ||
      reachableOps[2] != root.getOperation())
    return opRef.getOperation()->emitOpError(
        "scratch operations were not topologically sorted");

  opRef.getOperation()->emitRemark()
      << "reachable scratch collection ignored unused scratch op";
  opRef.getOperation()->emitRemark()
      << "reachable scratch DAG sorted dependencies before users";

  mlir::Operation *unusedOperation = unused.getOperation();
  transaction.discard();
  if (graph.getOperationOwnership(unusedOperation) !=
      mlir::egraph::OperationOwnership::IllegalExternal)
    return opRef.getOperation()->emitOpError(
        "unused scratch operation remained registered after discard");
  opRef.getOperation()->emitRemark()
      << "unused scratch cleanup unregistered ignored op";
  return mlir::success();
}

mlir::LogicalResult
verifyReachableScratchEquivalence(mlir::ModuleOp module,
                                  mlir::egraph::EGraph &graph,
                                  mlir::egraph::EOpRefBase fromRef) {
  mlir::egraph::EGraphRewriteTransaction transaction(graph,
                                                     module.getContext());
  ...
  mlir::Location loc = fromRef.getLoc();
  mlir::Type resultType = fromRef.getOperation()->getResult(0).getType();
  auto leaf = mlir::egraph::test::LeafOp::create(
      rewriter, loc, resultType,
      rewriter.getStringAttr("equivalence-reachable-leaf"));
  auto target = mlir::egraph::test::OpBOp::create(
      rewriter, loc, resultType, leaf.getResult(),
      rewriter.getStringAttr("equivalence-target"));

  if (mlir::failed(rewriter.replaceAllUsesWith(fromRef.getOperation(),
                                               target.getOperation())))
    return fromRef.getOperation()->emitOpError(
        "failed to record scratch equivalence target");

  mlir::FailureOr<llvm::SmallVector<mlir::Operation *>> reachable =
      transaction.collectReachableScratchOperations();
  if (mlir::failed(reachable))
    return fromRef.getOperation()->emitOpError(
        "failed to collect reachable equivalence target operations");

  llvm::ArrayRef<mlir::Operation *> reachableOps = *reachable;
  if (reachableOps.size() != 2 || reachableOps[0] != leaf.getOperation() ||
      reachableOps[1] != target.getOperation())
    return fromRef.getOperation()->emitOpError(
        "scratch equivalence target was not collected in topological order");

  fromRef.getOperation()->emitRemark()
      << "reachable scratch collection included equivalence target results";
  return mlir::success();
}

mlir::LogicalResult
verifyReachableScratchCollection(mlir::ModuleOp module,
                                 mlir::egraph::EGraph &graph) {
  mlir::egraph::EOpRefBase opARef;
  mlir::egraph::EOpRefBase opBRef;
  if (mlir::failed(findRewriteValidationRefs(module, graph, opARef, opBRef)))
  ...
  if (mlir::failed(verifyReachableScratchReplacement(module, graph, opARef)) ||
      mlir::failed(verifyReachableScratchEquivalence(module, graph, opBRef)))
    return mlir::failure();
...
mlir::FailureOr<mlir::egraph::EOpRefBase>
findOpAByMode(mlir::egraph::EGraph &graph, llvm::StringRef mode) {
  for (mlir::egraph::EOpRefBase ref : graph.getOpRefs()) {
    auto opA = llvm::dyn_cast<mlir::egraph::test::OpAOp>(ref.getOperation());
    if (!opA)
      continue;

    auto modeAttr = opA->getAttrOfType<mlir::StringAttr>("mode");
    if (modeAttr && modeAttr.getValue() == mode)
      return ref;
  }
...
mlir::LogicalResult verifyInternalInternDedup(mlir::ModuleOp module,
                                              mlir::egraph::EGraph &graph,
                                              mlir::egraph::EOpRefBase opARef) {
  mlir::egraph::EGraphRewriteTransaction transaction(graph,
                                                     module.getContext());
  ...
  auto scratch = mlir::egraph::test::OpAOp::create(
      rewriter, opARef.getLoc(), opARef.getOperation()->getResult(0).getType(),
      opARef.getOperation()->getOperand(0),
      opARef.getOperation()->getOperand(1), rewriter.getStringAttr("dedupe"));
  if (mlir::failed(rewriter.replaceOp(opARef, scratch->getResults())))
    return opARef.getOperation()->emitOpError(
        "failed to record duplicate scratch replacement");

  unsigned beforeCount = countEClasses(module);
  mlir::FailureOr<mlir::egraph::EGraphTransactionInternResult> interned =
      transaction.internReachableScratchOperations();
  if (mlir::failed(interned))
    return opARef.getOperation()->emitOpError(
        "failed to intern duplicate scratch operation");

  if (interned->operations.size() != 1 || interned->operations[0].inserted ||
      countEClasses(module) != beforeCount)
    return opARef.getOperation()->emitOpError(
        "duplicate scratch operation did not reuse hashcons entry");
  if (interned->eventValues.size() != 1 ||
      interned->eventValues[0].value != opARef.getResult(0).getLeader())
    return opARef.getOperation()->emitOpError(
        "duplicate scratch replacement resolved to the wrong evalue");

  transaction.discard();
  opARef.getOperation()->emitRemark()
      << "internal intern reused structural hashcons entry";
  return mlir::success();
}
...
mlir::LogicalResult verifyMatchPositiveNested(mlir::ModuleOp module,
                                              mlir::egraph::EGraph &graph) {
  mlir::FailureOr<mlir::egraph::EOpRefBase> parentRef =
      findOpBByTag(graph, "match_nested_parent");
  if (mlir::failed(parentRef))
    return module.emitError("expected nested match parent candidate");

  bool matchedNested = false;
  if (mlir::failed(parentRef->getOperand(0).matchDef<mlir::egraph::test::OpAOp>(
          [&](mlir::egraph::EOpRef<mlir::egraph::test::OpAOp> opA) {
            if (getOpAMode(opA) != "match_nested_child")
              return mlir::failure();
            matchedNested = true;
            return mlir::success();
          })) ||
      !matchedNested)
    return parentRef->getOperation()->emitOpError(
        "nested matchDef did not find op_a child");

  parentRef->getOperation()->emitRemark()
      << "match positives matched nested op_a";
  return mlir::success();
}

mlir::LogicalResult
verifyMatchPositiveEValueQueries(mlir::ModuleOp module,
                                 mlir::egraph::EGraph &graph) {
  mlir::FailureOr<mlir::egraph::EOpRefBase> childRef =
      findOpAByMode(graph, "match_nested_child");
  mlir::FailureOr<mlir::egraph::EOpRefBase> attrRef =
      findOpAByMode(graph, "match_attr_slow");
  if (mlir::failed(childRef) || mlir::failed(attrRef))
    return module.emitError("expected match positive op_a candidates");

  mlir::egraph::EValue value = childRef->getResult(0);
  bool sawEquivalentOpB = false;
  for (mlir::egraph::EOpRef<mlir::egraph::test::OpBOp> opB :
       value.getDefs<mlir::egraph::test::OpBOp>()) {
    auto tagAttr = opB.getOp()->getAttrOfType<mlir::StringAttr>("tag");
    sawEquivalentOpB |= tagAttr && tagAttr.getValue() == "match_equiv_b";
  }
  if (!sawEquivalentOpB)
    return childRef->getOperation()->emitOpError(
        "equivalent op_b candidate was not visible from evalue");

  llvm::SmallVector<mlir::egraph::EOpRef<mlir::egraph::test::OpAOp>> opADefs =
      value.getDefs<mlir::egraph::test::OpAOp>();
  if (opADefs.size() != 2)
    return childRef->getOperation()->emitOpError(
        "expected two same-type op_a candidates");
  if (childRef->getOperand(0) != attrRef->getOperand(1) ||
      childRef->getOperand(1) != attrRef->getOperand(0))
    return childRef->getOperation()->emitOpError(
        "same-type op_a candidates did not preserve distinct children");

  bool matchedAttribute = false;
  if (mlir::failed(value.matchDef<mlir::egraph::test::OpAOp>(
          [&](mlir::egraph::EOpRef<mlir::egraph::test::OpAOp> opA) {
            if (getOpAMode(opA) != "match_attr_slow")
              return mlir::failure();
      ...
      !matchedAttribute)
    return childRef->getOperation()->emitOpError(
        "attribute predicate did not select the slow op_a candidate");

  childRef->getOperation()->emitRemark()
      << "match positives observed equivalent candidates";
  childRef->getOperation()->emitRemark()
      << "match positives kept same-type candidates distinct";
  childRef->getOperation()->emitRemark()
      << "match positives selected candidate by attribute";
  return mlir::success();
}

mlir::LogicalResult
verifyMatchPositiveAliasCandidate(mlir::ModuleOp module,
                                  mlir::egraph::EGraph &graph) {
  mlir::FailureOr<mlir::egraph::EOpRefBase> aliasAddRef =
      findOpAByMode(graph, "match_alias_add_zero");
  if (mlir::failed(aliasAddRef))
    return module.emitError("expected alias add-zero candidate");

  bool matchedZero = false;
  if (mlir::failed(
          aliasAddRef->getOperand(1).matchDef<mlir::egraph::test::LeafOp>(
              [&](mlir::egraph::EOpRef<mlir::egraph::test::LeafOp> leaf) {
                auto nameAttr =
                    leaf.getOp()->getAttrOfType<mlir::StringAttr>("name");
                if (!nameAttr || nameAttr.getValue() != "zero")
                  return mlir::failure();
                matchedZero = true;
                return mlir::success();
              })) ||
      !matchedZero)
    return aliasAddRef->getOperation()->emitOpError(
        "add-zero candidate did not reference the zero leaf");

  mlir::egraph::EClassOp eclass = aliasAddRef->getEClassOp();
  mlir::egraph::EValue aliasedInput = aliasAddRef->getOperand(0).getLeader();
  bool foundAliasYield = false;
  for (mlir::Region &candidate : eclass.getCandidates()) {
    if (candidate.empty())
      continue;

    mlir::Block &block = candidate.front();
    auto yield =
        llvm::dyn_cast_or_null<mlir::egraph::YieldOp>(block.getTerminator());
    if (!yield || yield.getValues().size() != 1)
      continue;

    auto aliasArg = llvm::dyn_cast<mlir::BlockArgument>(yield.getValues()[0]);
    if (!aliasArg || aliasArg.getOwner() != &block)
      continue;
    if (!graph.isLegalAliasValue(aliasArg))
      return eclass.emitOpError("alias yield argument was not legal");

    mlir::FailureOr<mlir::egraph::EValue> aliasValue =
        graph.lookupValue(aliasArg);
    if (mlir::failed(aliasValue))
      return eclass.emitOpError("alias yield argument did not resolve");
    if (aliasValue->getLeader() != aliasedInput)
      return eclass.emitOpError("alias yield did not map back to x");

    foundAliasYield = true;
  }

  if (!foundAliasYield)
    return eclass.emitOpError("expected egraph.yield of an alias argument");

  aliasAddRef->getOperation()->emitRemark()
      << "match positives verified alias candidate yield";
  return mlir::success();
}

mlir::LogicalResult verifyMatchPositiveScenarios(mlir::ModuleOp module,
                                                 mlir::egraph::EGraph &graph) {
  if (mlir::failed(verifyMatchPositiveNested(module, graph)) ||
      mlir::failed(verifyMatchPositiveEValueQueries(module, graph)) ||
      mlir::failed(verifyMatchPositiveAliasCandidate(module, graph)))
    return mlir::failure();

  return mlir::success();
}

bool containsEClass(llvm::ArrayRef<mlir::egraph::EClassOp> eclasses,
                    mlir::egraph::EClassOp eclass) {
  return llvm::is_contained(eclasses, eclass);
}

mlir::LogicalResult
verifyTouchedEClasses(mlir::Operation *anchor,
                      const mlir::egraph::EGraphUnionResult &unioned,
                      llvm::ArrayRef<mlir::egraph::EClassOp> expectedEClasses) {
  for (mlir::egraph::EClassOp eclass : expectedEClasses) {
    if (!containsEClass(unioned.touchedEClasses, eclass))
      return anchor->emitOpError("union result did not mark touched eclass");
  }
  return mlir::success();
}

mlir::LogicalResult
verifyInternalUnionReplacement(mlir::ModuleOp module,
                               mlir::egraph::EGraph &graph,
                               mlir::egraph::EOpRefBase opARef) {
  graph.clearTouchedEClasses();
  mlir::egraph::EGraphRewriteTransaction transaction(graph,
  ...
  auto scratch = mlir::egraph::test::OpBOp::create(
      rewriter, opARef.getLoc(), opARef.getOperation()->getResult(0).getType(),
      opARef.getOperation()->getOperand(0),
      rewriter.getStringAttr("union-replacement"));
  if (mlir::failed(rewriter.replaceOp(opARef, scratch->getResults())))
    return opARef.getOperation()->emitOpError(
        "failed to record union replacement");

  mlir::FailureOr<mlir::egraph::EGraphTransactionInternResult> interned =
      transaction.internReachableScratchOperations();
  if (mlir::failed(interned) || interned->eventValues.size() != 1 ||
      interned->operations.size() != 1 || !interned->operations[0].inserted)
    return opARef.getOperation()->emitOpError(
        "failed to intern union replacement");

  mlir::egraph::EValue oldValue = opARef.getResult(0);
  mlir::egraph::EValue newValue = interned->eventValues[0].value;
  if (oldValue.getLeader() == newValue.getLeader())
    return opARef.getOperation()->emitOpError(
        "replacement values were unexpectedly equal before union");

  mlir::FailureOr<mlir::egraph::EGraphUnionResult> unioned =
      transaction.unionInternedEventValues(*interned);
  if (mlir::failed(unioned) || !unioned->changed)
    return opARef.getOperation()->emitOpError(
        "failed to union replacement event values");

  if (oldValue.getLeader() != oldValue || newValue.getLeader() != oldValue)
    return opARef.getOperation()->emitOpError(
        "replacement union did not keep the old result as leader");

  if (mlir::failed(verifyTouchedEClasses(
          opARef.getOperation(), *unioned,
          {opARef.getEClassOp(), interned->operations[0].eclass})))
    return mlir::failure();
  if (!containsEClass(graph.getTouchedEClasses(), opARef.getEClassOp()) ||
      !containsEClass(graph.getTouchedEClasses(),
                      interned->operations[0].eclass))
    return opARef.getOperation()->emitOpError(
        "graph did not retain touched eclasses");

  transaction.discard();
  opARef.getOperation()->emitRemark()
      << "internal union merged replacement result";
  return mlir::success();
}

mlir::LogicalResult verifyInternalUnionEquivalence(
    mlir::ModuleOp module, mlir::egraph::EGraph &graph,
    mlir::egraph::EOpRefBase fromRef, mlir::egraph::EOpRefBase toRef) {
  graph.clearTouchedEClasses();
  mlir::egraph::EGraphRewriteTransaction transaction(graph,
  ...
  if (mlir::failed(rewriter.replaceAllUsesWith(fromRef, toRef)))
    return fromRef.getOperation()->emitOpError(
        "failed to record union equivalence");

  mlir::FailureOr<mlir::egraph::EGraphTransactionInternResult> interned =
      transaction.internReachableScratchOperations();
  if (mlir::failed(interned) || !interned->operations.empty() ||
      interned->eventValues.size() != 1)
    return fromRef.getOperation()->emitOpError(
        "failed to resolve union equivalence target");

  mlir::egraph::EValue fromValue = fromRef.getResult(0);
  mlir::egraph::EValue toValue = toRef.getResult(0);
  if (fromValue.getLeader() == toValue.getLeader())
    return fromRef.getOperation()->emitOpError(
        "equivalence values were unexpectedly equal before union");

  mlir::FailureOr<mlir::egraph::EGraphUnionResult> unioned =
      transaction.unionInternedEventValues(*interned);
  if (mlir::failed(unioned) || !unioned->changed)
    return fromRef.getOperation()->emitOpError(
        "failed to union equivalence event values");

  if (fromValue.getLeader() != fromValue || toValue.getLeader() != fromValue)
    return fromRef.getOperation()->emitOpError(
        "equivalence union did not keep the from result as leader");

  if (mlir::failed(
          verifyTouchedEClasses(fromRef.getOperation(), *unioned,
                                {fromRef.getEClassOp(), toRef.getEClassOp()})))
    return mlir::failure();

  transaction.discard();
  fromRef.getOperation()->emitRemark()
      << "internal union merged equivalence result";
  return mlir::success();
}

mlir::LogicalResult verifyInternalUnionMultiResult(
    mlir::ModuleOp module, mlir::egraph::EGraph &graph,
    mlir::egraph::EOpRefBase i32Ref,
    llvm::ArrayRef<mlir::egraph::EOpRefBase> splitRefs) {
  if (splitRefs.size() < 2)
    return module.emitError("expected two split candidates");

  mlir::egraph::EOpRefBase lhsSplit = splitRefs[0];
  mlir::egraph::EOpRefBase rhsSplit = splitRefs[1];
  llvm::SmallVector<mlir::egraph::EValue> lhsValues = {lhsSplit.getResult(0),
                                                       lhsSplit.getResult(1)};
  llvm::SmallVector<mlir::egraph::EValue> rhsValues = {rhsSplit.getResult(0),
                                                       rhsSplit.getResult(1)};

  graph.clearTouchedEClasses();
  mlir::FailureOr<mlir::egraph::EGraphUnionResult> unioned =
      graph.unionValueTuples(lhsValues, rhsValues);
  if (mlir::failed(unioned) || !unioned->changed)
    return lhsSplit.getOperation()->emitOpError(
        "failed to union multi-result tuple");

  if (rhsValues[0].getLeader() != lhsValues[0] ||
      rhsValues[1].getLeader() != lhsValues[1])
    return lhsSplit.getOperation()->emitOpError(
        "multi-result tuple union lost slot-wise leader values");

  if (mlir::failed(verifyTouchedEClasses(
          lhsSplit.getOperation(), *unioned,
          {lhsSplit.getEClassOp(), rhsSplit.getEClassOp()})))
    return mlir::failure();
  lhsSplit.getOperation()->emitRemark()
      << "internal union merged multi-result tuple";

  if (mlir::succeeded(
          graph.unionValues(i32Ref.getResult(0), lhsSplit.getResult(0))))
    return i32Ref.getOperation()->emitOpError(
        "union accepted mismatched payload types");
  i32Ref.getOperation()->emitRemark()
      << "internal union rejected type mismatch";

  if (mlir::succeeded(
          graph.unionValues(lhsSplit.getResult(0), lhsSplit.getResult(1))))
    return lhsSplit.getOperation()->emitOpError(
        "union accepted mismatched result slots");
  lhsSplit.getOperation()->emitRemark()
      << "internal union rejected result slot mismatch";

  return mlir::success();
}

mlir::LogicalResult verifyInternalUnion(mlir::ModuleOp module,
                                        mlir::egraph::EGraph &graph) {
  mlir::FailureOr<mlir::egraph::EOpRefBase> replacementRef =
      findOpAByMode(graph, "union_replace");
  mlir::FailureOr<mlir::egraph::EOpRefBase> equivalenceToRef =
      findOpAByMode(graph, "union_equiv_to");
  mlir::FailureOr<mlir::egraph::EOpRefBase> equivalenceFromRef =
      findOpBByTag(graph, "union_equiv_from");
  if (mlir::failed(replacementRef) || mlir::failed(equivalenceToRef) ||
      mlir::failed(equivalenceFromRef))
    return module.emitError("expected internal union test candidates");

  llvm::SmallVector<mlir::egraph::EOpRefBase> splitRefs = findSplitRefs(graph);
  if (mlir::failed(
          verifyInternalUnionReplacement(module, graph, *replacementRef)) ||
      mlir::failed(verifyInternalUnionEquivalence(
          module, graph, *equivalenceFromRef, *equivalenceToRef)) ||
      mlir::failed(verifyInternalUnionMultiResult(module, graph,
                                                  *replacementRef, splitRefs)))
    return mlir::failure();

  return mlir::success();
}

unsigned countOpAByMode(mlir::egraph::EGraph &graph, llvm::StringRef mode) {
  unsigned count = 0;
  for (mlir::egraph::EOpRefBase ref : graph.getOpRefs()) {
    auto opA = llvm::dyn_cast<mlir::egraph::test::OpAOp>(ref.getOperation());
    if (!opA)
...
bool containsOpBWithTag(llvm::ArrayRef<mlir::egraph::EOpRefBase> refs,
                        llvm::StringRef tag) {
  for (mlir::egraph::EOpRefBase ref : refs) {
    auto opB = llvm::dyn_cast<mlir::egraph::test::OpBOp>(ref.getOperation());
    if (!opB)
      continue;
...
bool containsOpAWithMode(llvm::ArrayRef<mlir::egraph::EOpRefBase> refs,
                         llvm::StringRef mode) {
  for (mlir::egraph::EOpRefBase ref : refs)
    if (llvm::isa<mlir::egraph::test::OpAOp>(ref.getOperation()) &&
        getOpAMode(ref) == mode)
...
mlir::LogicalResult verifyFullRebuild(mlir::ModuleOp module,
                                      mlir::egraph::EGraph &graph) {
  mlir::FailureOr<mlir::egraph::EOpRefBase> lhsRef =
      findOpBByTag(graph, "rebuild_lhs");
  mlir::FailureOr<mlir::egraph::EOpRefBase> rhsRef =
      findOpBByTag(graph, "rebuild_rhs");
  mlir::FailureOr<mlir::egraph::EOpRefBase> affectedRhsRef =
      findOpBByTag(graph, "affected_rhs");
  if (mlir::failed(lhsRef) || mlir::failed(rhsRef) ||
      mlir::failed(affectedRhsRef))
    return module.emitError("expected full rebuild test candidates");

  mlir::egraph::EValue lhsValue = lhsRef->getResult(0);
  mlir::egraph::EValue rhsValue = rhsRef->getResult(0);
  mlir::egraph::EOpRefBase duplicateRhsRef;
  for (mlir::egraph::EOpRefBase ref : graph.getOpRefs()) {
    auto opA = llvm::dyn_cast<mlir::egraph::test::OpAOp>(ref.getOperation());
    if (!opA || getOpAMode(ref) != "rebuild_dedup")
      continue;
    if (ref.getOperand(0) == rhsValue) {
      duplicateRhsRef = ref;
      break;
    }
  }
  if (!duplicateRhsRef)
    return module.emitError("expected full rebuild duplicate rhs candidate");

  graph.clearTouchedEClasses();
  mlir::FailureOr<mlir::egraph::EGraphUnionResult> unioned =
      graph.unionValues(lhsValue, rhsValue);
  if (mlir::failed(unioned) || !unioned->changed)
    return lhsRef->getOperation()->emitOpError(
        "failed to union full rebuild inputs");

  mlir::Operation *duplicateRhsOperation = duplicateRhsRef.getOperation();
  mlir::FailureOr<mlir::egraph::EGraphRebuildResult> rebuilt =
      graph.rebuild(module);
  if (mlir::failed(rebuilt))
    return module.emitError("full rebuild failed");

  if (affectedRhsRef->isLive())
    return affectedRhsRef->getOperation()->emitOpError(
        "stale worklist ref stayed live after full rebuild");
  module.emitRemark() << "full rebuild made old worklist refs stale";

  if (rebuilt->newCandidateRoots.size() != graph.getOpRefs().size() ||
      rebuilt->newCandidateRoots.empty())
    return module.emitError("full rebuild did not return candidate roots");
  module.emitRemark() << "full rebuild returned "
                      << rebuilt->newCandidateRoots.size()
                      << " candidate root(s)";

  if (!containsOpBWithTag(rebuilt->affectedParentCandidates, "affected_lhs") ||
      !containsOpBWithTag(rebuilt->affectedParentCandidates, "affected_rhs"))
    return module.emitError("full rebuild lost affected parent candidates");
  module.emitRemark()
      << "full rebuild returned affected parent candidates after union";

  llvm::SmallVector<mlir::egraph::EOpRef<mlir::egraph::test::OpBOp>> lhsDefs =
      lhsValue.getDefs<mlir::egraph::test::OpBOp>();
  bool sawLhs = false;
  bool sawRhs = false;
  for (mlir::egraph::EOpRef<mlir::egraph::test::OpBOp> ref : lhsDefs) {
    auto tagAttr = ref.getOp()->getAttrOfType<mlir::StringAttr>("tag");
    if (!tagAttr)
      continue;
    sawLhs |= tagAttr.getValue() == "rebuild_lhs";
    sawRhs |= tagAttr.getValue() == "rebuild_rhs";
  }
  if (!sawLhs || !sawRhs)
    return module.emitError("leader evalue did not expose unioned definitions");
  module.emitRemark() << "full rebuild remapped defs to leader evalue";

  if (countOpAByMode(graph, "rebuild_dedup") != 1 ||
      mlir::succeeded(graph.lookupOpRef(duplicateRhsOperation)))
    return duplicateRhsOperation->emitOpError(
        "full rebuild did not mark duplicate structural candidate dead");
  duplicateRhsOperation->emitRemark()
      << "full rebuild removed duplicate structural candidate";

  return mlir::success();
}

mlir::LogicalResult verifyCommitFailureRollback(mlir::ModuleOp module,
                                                mlir::egraph::EGraph &graph,
                                                mlir::egraph::EOpRefBase ref) {
  unsigned initialRefCount = graph.getOpRefs().size();
  unsigned initialEClassCount = countEClasses(module);
  ...
  auto scratch = mlir::egraph::test::LeafOp::create(
      rewriter, ref.getLoc(), ref.getOperation()->getResult(0).getType(),
      rewriter.getStringAttr("commit-failure-rollback"));
  mlir::Operation *scratchOperation = scratch.getOperation();
  transaction.discard();

  if (!transaction.isDiscarded() || transaction.getScratchBlock())
    return module.emitError("failed transaction did not release scratch block");
  if (graph.getOperationOwnership(scratchOperation) !=
      mlir::egraph::OperationOwnership::IllegalExternal)
    return module.emitError("failed transaction left scratch ownership");
  if (graph.getOpRefs().size() != initialRefCount ||
      countEClasses(module) != initialEClassCount)
    return module.emitError("failed transaction changed persistent egraph");

  module.emitRemark() << "transaction failure rollback discarded scratch state";
  return mlir::success();
}

mlir::LogicalResult verifyCommitNoEvent(mlir::ModuleOp module,
                                        mlir::egraph::EGraph &graph,
                                        mlir::egraph::EOpRefBase ref) {
  unsigned initialRefCount = graph.getOpRefs().size();
  unsigned initialEClassCount = countEClasses(module);
  ...
  auto scratch = mlir::egraph::test::LeafOp::create(
      rewriter, ref.getLoc(), ref.getOperation()->getResult(0).getType(),
      rewriter.getStringAttr("commit-no-event"));
  mlir::Operation *scratchOperation = scratch.getOperation();

  mlir::FailureOr<mlir::egraph::EGraphRewriteCommitResult> committed =
      transaction.commit(module);
  if (mlir::failed(committed))
    return module.emitError("no-event transaction commit failed");
  if (committed->changed || !committed->newCandidateRoots.empty() ||
      !committed->affectedParentCandidates.empty())
    return module.emitError("no-event transaction reported changed work");
  if (!transaction.isDiscarded() || transaction.getScratchBlock())
    return module.emitError("no-event commit did not release scratch block");
  if (graph.getOperationOwnership(scratchOperation) !=
      mlir::egraph::OperationOwnership::IllegalExternal)
    return module.emitError("no-event commit left scratch ownership");
  if (graph.getOpRefs().size() != initialRefCount ||
      countEClasses(module) != initialEClassCount)
    return module.emitError("no-event commit changed persistent egraph");

  module.emitRemark() << "transaction commit discarded success without events";
  return mlir::success();
}

mlir::LogicalResult
verifyCommitValidationFailure(mlir::ModuleOp module,
                              mlir::egraph::EGraph &graph,
                              mlir::egraph::EOpRefBase ref) {
  unsigned initialRefCount = graph.getOpRefs().size();
  unsigned initialEClassCount = countEClasses(module);
  ...
  auto scratch = mlir::egraph::test::LeafOp::create(
      rewriter, ref.getLoc(), rewriter.getI64Type(),
      rewriter.getStringAttr("commit-validation-failure"));
  mlir::Operation *scratchOperation = scratch.getOperation();
  if (mlir::failed(rewriter.replaceOp(ref, scratch->getResults())))
    return ref.getOperation()->emitOpError(
        "failed to record invalid commit replacement");

  if (mlir::succeeded(transaction.commit(module)))
    return module.emitError("invalid transaction commit succeeded");
  if (!transaction.isDiscarded() || transaction.getScratchBlock())
    return module.emitError("invalid commit did not release scratch block");
  if (graph.getOperationOwnership(scratchOperation) !=
      mlir::egraph::OperationOwnership::IllegalExternal)
    return module.emitError("invalid commit left scratch ownership");
  if (graph.getOpRefs().size() != initialRefCount ||
      countEClasses(module) != initialEClassCount)
    return module.emitError("invalid commit changed persistent egraph");

  module.emitRemark() << "transaction commit validation failure rolled back";
  return mlir::success();
}

mlir::LogicalResult verifyCommitDedupNoChange(mlir::ModuleOp module,
                                              mlir::egraph::EGraph &graph,
                                              mlir::egraph::EOpRefBase ref) {
  unsigned initialRefCount = graph.getOpRefs().size();
  unsigned initialEClassCount = countEClasses(module);
  mlir::egraph::EValue oldValue = ref.getResult(0);
  mlir::egraph::EGraphRewriteTransaction transaction(graph,
                                                     module.getContext());
  mlir::egraph::EGraphPatternRewriter rewriter(transaction);
  ...
  auto scratch = mlir::egraph::test::OpAOp::create(
      rewriter, ref.getLoc(), ref.getOperation()->getResult(0).getType(),
      ref.getOperation()->getOperand(0), ref.getOperation()->getOperand(1),
      rewriter.getStringAttr("commit_dedupe"));
  mlir::Operation *scratchOperation = scratch.getOperation();
  if (mlir::failed(rewriter.replaceOp(ref, scratch->getResults())))
    return ref.getOperation()->emitOpError(
        "failed to record duplicate commit replacement");

  mlir::FailureOr<mlir::egraph::EGraphRewriteCommitResult> committed =
      transaction.commit(module);
  if (mlir::failed(committed))
    return module.emitError("duplicate transaction commit failed");
  if (committed->changed || !committed->newCandidateRoots.empty() ||
      !committed->affectedParentCandidates.empty())
    return module.emitError("duplicate transaction reported changed work");
  if (!transaction.isDiscarded() || transaction.getScratchBlock())
    return module.emitError("duplicate commit did not release scratch block");
  if (graph.getOperationOwnership(scratchOperation) !=
      mlir::egraph::OperationOwnership::IllegalExternal)
    return module.emitError("duplicate commit left scratch ownership");
  if (!ref.isLive())
    return module.emitError("duplicate commit made unchanged ref stale");
  if (graph.getOpRefs().size() != initialRefCount ||
      countEClasses(module) != initialEClassCount)
    return module.emitError("duplicate commit changed persistent egraph");
  if (oldValue.getLeader() != oldValue)
    return module.emitError("duplicate commit unexpectedly changed leader");

  module.emitRemark() << "transaction commit reused duplicate intern result";
  return mlir::success();
}

mlir::LogicalResult verifyCommitPositive(mlir::ModuleOp module,
                                         mlir::egraph::EGraph &graph,
                                         mlir::egraph::EOpRefBase ref) {
  unsigned initialEClassCount = countEClasses(module);
  mlir::egraph::EValue oldValue = ref.getResult(0);
  ...
  auto scratch = mlir::egraph::test::OpBOp::create(
      rewriter, ref.getLoc(), ref.getOperation()->getResult(0).getType(),
      ref.getOperation()->getOperand(0), rewriter.getStringAttr("commit_new"));
  mlir::Operation *scratchOperation = scratch.getOperation();
  if (mlir::failed(rewriter.replaceOp(ref, scratch->getResults())))
    return ref.getOperation()->emitOpError(
        "failed to record positive commit replacement");

  mlir::FailureOr<mlir::egraph::EGraphRewriteCommitResult> committed =
      transaction.commit(module);
  if (mlir::failed(committed) || !committed->changed)
    return module.emitError("positive transaction commit did not change");
  if (!transaction.isDiscarded() || transaction.getScratchBlock())
    return module.emitError("positive commit did not release scratch block");
  if (graph.getOperationOwnership(scratchOperation) !=
      mlir::egraph::OperationOwnership::IllegalExternal)
    return module.emitError("positive commit left scratch ownership");
  if (ref.isLive())
    return module.emitError("positive commit left old occurrence live");
  if (countEClasses(module) != initialEClassCount + 1)
    return module.emitError("positive commit did not intern one new eclass");
  if (committed->newCandidateRoots.empty() ||
      !containsOpBWithTag(committed->newCandidateRoots, "commit_new"))
    return module.emitError("positive commit did not return new roots");
  if (!containsOpBWithTag(committed->affectedParentCandidates, "commit_parent"))
    return module.emitError("positive commit lost affected parent candidates");

  bool sawCommittedDef = false;
  for (mlir::egraph::EOpRef<mlir::egraph::test::OpBOp> def :
       oldValue.getDefs<mlir::egraph::test::OpBOp>()) {
    auto tagAttr = def.getOp()->getAttrOfType<mlir::StringAttr>("tag");
    sawCommittedDef |= tagAttr && tagAttr.getValue() == "commit_new";
  }
  if (!sawCommittedDef)
    return module.emitError("positive commit did not expose committed def");

  module.emitRemark() << "transaction commit merged replacement event";
  module.emitRemark() << "transaction commit rebuilt changed egraph";
  module.emitRemark()
      << "transaction commit returned affected parent candidates";
  return mlir::success();
}

mlir::LogicalResult verifyCommitEquivalence(mlir::ModuleOp module,
                                            mlir::egraph::EGraph &graph,
                                            mlir::egraph::EOpRefBase fromRef,
                                            mlir::egraph::EOpRefBase toRef) {
  unsigned initialEClassCount = countEClasses(module);
  mlir::egraph::EValue fromValue = fromRef.getResult(0);
  mlir::egraph::EValue toValue = toRef.getResult(0);
  mlir::egraph::EGraphRewriteTransaction transaction(graph,
  ...
  if (mlir::failed(rewriter.replaceAllUsesWith(fromRef, toRef)))
    return fromRef.getOperation()->emitOpError(
        "failed to record commit equivalence event");

  mlir::FailureOr<mlir::egraph::EGraphRewriteCommitResult> committed =
      transaction.commit(module);
  if (mlir::failed(committed) || !committed->changed)
    return module.emitError("equivalence transaction commit did not change");
  if (!transaction.isDiscarded() || transaction.getScratchBlock())
    return module.emitError("equivalence commit did not release scratch block");
  if (fromRef.isLive())
    return module.emitError("equivalence commit left old occurrence live");
  if (countEClasses(module) != initialEClassCount)
    return module.emitError("equivalence commit changed eclass count");
  if (toValue.getLeader() != fromValue)
    return module.emitError("equivalence commit did not union target value");

  bool sawEquivalentDef = false;
  for (mlir::egraph::EOpRef<mlir::egraph::test::OpAOp> def :
       fromValue.getDefs<mlir::egraph::test::OpAOp>())
    sawEquivalentDef |= getOpAMode(def) == "commit_equiv_to";
  if (!sawEquivalentDef)
    return module.emitError("equivalence commit did not expose target def");

  module.emitRemark() << "transaction commit merged equivalence event";
  return mlir::success();
}

mlir::LogicalResult
verifyCommitSingleResultScratchDAG(mlir::ModuleOp module,
                                   mlir::egraph::EGraph &graph,
                                   mlir::egraph::EOpRefBase ref) {
  unsigned initialEClassCount = countEClasses(module);
  mlir::egraph::EValue oldValue = ref.getResult(0);
  ...
  mlir::Location loc = ref.getLoc();
  mlir::Type resultType = ref.getOperation()->getResult(0).getType();
  auto unused = mlir::egraph::test::LeafOp::create(
      rewriter, loc, resultType, rewriter.getStringAttr("commit-unused"));
  mlir::Operation *unusedOperation = unused.getOperation();
  auto mid = mlir::egraph::test::OpBOp::create(
      rewriter, loc, resultType, ref.getOperation()->getOperand(0),
      rewriter.getStringAttr("commit_dag_mid"));
  auto root = mlir::egraph::test::OpAOp::create(
      rewriter, loc, resultType, mid.getResult(),
      ref.getOperation()->getOperand(1),
      rewriter.getStringAttr("commit_dag_root"));
  if (mlir::failed(rewriter.replaceOp(ref, root->getResults())))
    return ref.getOperation()->emitOpError(
        "failed to record single-result scratch DAG replacement");

  mlir::FailureOr<mlir::egraph::EGraphRewriteCommitResult> committed =
      transaction.commit(module);
  if (mlir::failed(committed) || !committed->changed)
    return module.emitError("scratch DAG transaction commit did not change");
  if (!transaction.isDiscarded() || transaction.getScratchBlock())
    return module.emitError("scratch DAG commit did not release scratch block");
  if (graph.getOperationOwnership(unusedOperation) !=
      mlir::egraph::OperationOwnership::IllegalExternal)
    return module.emitError("scratch DAG commit left unused scratch ownership");
  if (ref.isLive())
    return module.emitError("scratch DAG commit left old occurrence live");
  if (countEClasses(module) != initialEClassCount + 2)
    return module.emitError("scratch DAG commit did not split into eclasses");
  if (!containsOpAWithMode(committed->newCandidateRoots, "commit_dag_root") ||
      !containsOpBWithTag(committed->newCandidateRoots, "commit_dag_mid"))
    return module.emitError("scratch DAG commit did not return new roots");

  bool sawRootDef = false;
  for (mlir::egraph::EOpRef<mlir::egraph::test::OpAOp> def :
       oldValue.getDefs<mlir::egraph::test::OpAOp>())
    sawRootDef |= getOpAMode(def) == "commit_dag_root";
  if (!sawRootDef)
    return module.emitError("scratch DAG commit did not expose root def");

  module.emitRemark() << "transaction commit cleaned unused scratch op";
  module.emitRemark() << "transaction commit split single-result scratch DAG";
  return mlir::success();
}

mlir::LogicalResult
verifyCommitMultiResultLock(mlir::ModuleOp module, mlir::egraph::EGraph &graph,
                            mlir::egraph::EOpRefBase splitRef) {
  unsigned initialEClassCount = countEClasses(module);
  mlir::egraph::EValue firstValue = splitRef.getResult(0);
  mlir::egraph::EValue secondValue = splitRef.getResult(1);
  mlir::egraph::EGraphRewriteTransaction transaction(graph,
                                                     module.getContext());
  mlir::egraph::EGraphPatternRewriter rewriter(transaction);

  mlir::Value input = splitRef.getOperation()->getOperand(0);
  auto slice0 = mlir::egraph::test::Slice0Op::create(
      rewriter, splitRef.getLoc(),
      splitRef.getOperation()->getResult(0).getType(), input);
  auto slice1 = mlir::egraph::test::Slice1Op::create(
      rewriter, splitRef.getLoc(),
      splitRef.getOperation()->getResult(1).getType(), input);

  llvm::SmallVector<mlir::Value, 2> replacements = {slice0.getResult(),
                                                    slice1.getResult()};
  if (mlir::failed(rewriter.replaceOp(splitRef, replacements)))
    return splitRef.getOperation()->emitOpError(
        "failed to record multi-result tuple replacement");

  mlir::FailureOr<mlir::egraph::EGraphRewriteCommitResult> committed =
      transaction.commit(module);
  if (mlir::failed(committed) || !committed->changed)
    return module.emitError("multi-result transaction commit did not change");
  if (!transaction.isDiscarded() || transaction.getScratchBlock())
    return module.emitError(
        "multi-result commit did not release scratch block");
  if (splitRef.isLive())
    return module.emitError("multi-result commit left old occurrence live");
  if (countEClasses(module) != initialEClassCount + 1)
    return module.emitError("multi-result commit did not create tuple eclass");

  llvm::SmallVector<mlir::egraph::EOpRef<mlir::egraph::test::Slice0Op>>
      firstSlice0Defs = firstValue.getDefs<mlir::egraph::test::Slice0Op>();
  llvm::SmallVector<mlir::egraph::EOpRef<mlir::egraph::test::Slice1Op>>
      secondSlice1Defs = secondValue.getDefs<mlir::egraph::test::Slice1Op>();

  if (firstSlice0Defs.size() != 1 || secondSlice1Defs.size() != 1 ||
      firstSlice0Defs.front().getEClassOp() !=
          secondSlice1Defs.front().getEClassOp() ||
      !firstSlice0Defs.front().isSameCandidateAs(secondSlice1Defs.front()))
    return module.emitError("multi-result commit lost candidate lock");

  if (!firstValue.getDefs<mlir::egraph::test::Slice1Op>().empty() ||
      !secondValue.getDefs<mlir::egraph::test::Slice0Op>().empty())
    return module.emitError("multi-result commit mixed result slots");

  module.emitRemark() << "transaction commit preserved multi-result lock";
  return mlir::success();
}

mlir::LogicalResult verifyTransactionCommit(mlir::ModuleOp module,
                                            mlir::egraph::EGraph &graph) {
  mlir::FailureOr<mlir::egraph::EOpRefBase> replaceRef =
      findOpAByMode(graph, "commit_replace");
  mlir::FailureOr<mlir::egraph::EOpRefBase> dedupeRef =
      findOpAByMode(graph, "commit_dedupe");
  if (mlir::failed(replaceRef))
    return module.emitError(
        "expected transaction commit replacement candidate");
  if (mlir::failed(dedupeRef))
    return module.emitError("expected transaction commit dedupe candidate");

  if (mlir::failed(verifyCommitFailureRollback(module, graph, *replaceRef)) ||
      mlir::failed(verifyCommitNoEvent(module, graph, *replaceRef)) ||
      mlir::failed(verifyCommitValidationFailure(module, graph, *replaceRef)) ||
      mlir::failed(verifyCommitDedupNoChange(module, graph, *dedupeRef)) ||
      mlir::failed(verifyCommitPositive(module, graph, *replaceRef)))
    return mlir::failure();

  mlir::FailureOr<mlir::egraph::EOpRefBase> equivalenceFromRef =
      findOpBByTag(graph, "commit_equiv_from");
  mlir::FailureOr<mlir::egraph::EOpRefBase> equivalenceToRef =
      findOpAByMode(graph, "commit_equiv_to");
  if (mlir::failed(equivalenceFromRef) || mlir::failed(equivalenceToRef))
    return module.emitError(
        "expected transaction commit equivalence candidate");
  if (mlir::failed(verifyCommitEquivalence(module, graph, *equivalenceFromRef,
                                           *equivalenceToRef)))
    return mlir::failure();

  mlir::FailureOr<mlir::egraph::EOpRefBase> dagRef =
      findOpAByMode(graph, "commit_dag_replace");
  if (mlir::failed(dagRef))
    return module.emitError("expected transaction commit DAG candidate");
  if (mlir::failed(verifyCommitSingleResultScratchDAG(module, graph, *dagRef)))
    return mlir::failure();

  llvm::SmallVector<mlir::egraph::EOpRefBase> splitRefs = findSplitRefs(graph);
  if (splitRefs.empty())
    return module.emitError("expected transaction commit split candidate");
  if (mlir::failed(
          verifyCommitMultiResultLock(module, graph, splitRefs.front())))
    return mlir::failure();
...
mlir::LogicalResult verifyWorklistDriverIterationLimit(
  ...
  mlir::egraph::EGraphRewriteDriverConfig config;
  config.maxIterations = 1;

  mlir::FailureOr<mlir::egraph::EGraphRewriteDriverResult> driven =
  ...
  if (!driven->limitReached ||
      driven->reachedLimit != mlir::egraph::EGraphRewriteDriverLimit::Iteration)
...
mlir::LogicalResult verifyWorklistDriverCandidateLimit(
  ...
  if (!driven->limitReached ||
      driven->reachedLimit != mlir::egraph::EGraphRewriteDriverLimit::Candidate)
...
verifySymbolicWorklistDriverRebuildLimit(mlir::ModuleOp module) {
  ...
  if (!driven->limitReached ||
      driven->reachedLimit != mlir::egraph::EGraphRewriteDriverLimit::Rebuild)
...
mlir::LogicalResult verifyWorklistDriver(mlir::ModuleOp module,
  ...
  debugTrace.flush();
  if (driven->limitReached ||
      driven->reachedLimit != mlir::egraph::EGraphRewriteDriverLimit::None)
...
struct FoldConflictAddToAliasPattern final
    : public mlir::egraph::EGraphPatternFor<mlir::arith::AddIOp> {
  explicit FoldConflictAddToAliasPattern(unsigned &matchCount)
      : matchCount(matchCount) {}

  mlir::LogicalResult
  matchAndRewrite(mlir::egraph::EOpRef<mlir::arith::AddIOp> root,
                  mlir::egraph::EGraphPatternRewriter &rewriter) const final {
    ++matchCount;
    return rewriter.replaceOp(root, root.getOperation()->getOperand(0));
  }
...
mlir::LogicalResult runImportedPipeline(mlir::ModuleOp module) {
  ...
  mlir::PassManager importPM(module.getContext());
  importPM.addPass(mlir::egraph::createConvertFuncToEGraphPass());
  if (mlir::failed(importPM.run(module)))
    return module.emitError("failed to import demo function into egraph");

  auto egraph =
      module.lookupSymbol<mlir::egraph::EGraphOp>("arith_demo_egraph");
  if (!egraph)
    return module.emitError(
        "expected convert-func-to-egraph to create @arith_demo_egraph");

  auto input = egraph.lookupSymbol<mlir::egraph::InputOp>("arg0");
  auto root = egraph.lookupSymbol<mlir::egraph::EClassOp>("divsi");
  if (!input || !root)
    return egraph.emitOpError("expected imported @arg0 input and @divsi root");
  mlir::StringAttr inputName = input->getAttrOfType<mlir::StringAttr>(
      mlir::SymbolTable::getSymbolAttrName());
  mlir::StringAttr rootName = root->getAttrOfType<mlir::StringAttr>(
      mlir::SymbolTable::getSymbolAttrName());
  if (!inputName || !rootName)
    return egraph.emitOpError(
        "expected imported symbols to carry sym_name attrs");

  mlir::egraph::EGraph graph;
  if (mlir::failed(graph.indexEGraph(egraph)))
    return egraph.emitOpError("failed to index imported demo egraph");
  if (!graph.isClean())
    return egraph.emitOpError("expected imported demo egraph to start clean");
  ...
  if (mlir::failed(driven))
    return module.emitError("egraph pipeline driver failed");
  if (!graph.isClean())
    return module.emitError("egraph pipeline left the imported graph dirty");
  if (!driven->changed || driven->changedCommits == 0)
    return module.emitError("egraph pipeline did not change the demo graph");
  if (mulByTwoMatches == 0 || reassociateMatches == 0 || divSelfMatches == 0 ||
      mulByOneMatches == 0)
    return module.emitError(
        "egraph pipeline did not exercise all demo rewrite patterns");
  if (!graph.getValue(rootName).isEquivalentTo(graph.getValue(inputName)))
    return module.emitError(
        "egraph pipeline did not simplify the returned demo root to @arg0");

  auto persistentRoot = mlir::dyn_cast_or_null<mlir::egraph::EClassOp>(
      egraph.lookupSymbol<mlir::egraph::EClassOp>(rootName));
  if (!persistentRoot)
    return egraph.emitOpError("expected persistent @divsi after rebuild");

  bool foundInputAlias = false;
  mlir::ArrayAttr candidateRefs = persistentRoot.getCandidateRefs();
  for (auto indexedCandidate :
       llvm::enumerate(persistentRoot.getCandidates())) {
    unsigned candidateOrdinal = indexedCandidate.index();
    if (candidateOrdinal >= candidateRefs.size())
      return persistentRoot.emitOpError(
          "missing persistent candidate_refs entry");

    auto row = mlir::dyn_cast<mlir::ArrayAttr>(candidateRefs[candidateOrdinal]);
    if (!row)
      return persistentRoot.emitOpError(
          "persistent candidate_refs entry was malformed");
    if (row.size() != 1)
      continue;

    auto childRef = mlir::dyn_cast<mlir::FlatSymbolRefAttr>(row[0]);
    if (!childRef)
      return persistentRoot.emitOpError(
          "persistent alias candidate child ref was malformed");
    if (childRef.getAttr() != inputName)
      continue;

    mlir::Region &candidate = indexedCandidate.value();
    if (candidate.empty() || !llvm::hasSingleElement(candidate))
      return persistentRoot.emitOpError(
          "persistent alias candidate was malformed");
    mlir::Block &block = candidate.front();
    if (block.getNumArguments() != 1)
      return persistentRoot.emitOpError(
          "persistent alias candidate did not keep a single argument");
    auto yield =
        llvm::dyn_cast_or_null<mlir::egraph::YieldOp>(block.getTerminator());
    if (!yield || yield.getValues().size() != 1 ||
        yield.getValues()[0] != block.getArgument(0))
      return persistentRoot.emitOpError(
          "persistent alias candidate did not yield arg0");

    foundInputAlias = true;
    break;
  }
  if (!foundInputAlias)
    return persistentRoot.emitOpError(
        "expected persistent alias candidate for @arg0");

  module.emitRemark() << "egraph pipeline imported arith demo";
  module.emitRemark() << "egraph pipeline ran symbolic rewrite driver";
  return mlir::success();
}

mlir::LogicalResult runImportedFoldPipeline(mlir::ModuleOp module) {
  auto foldFunc = module.lookupSymbol<mlir::func::FuncOp>("fold_demo");
  auto foldValueFunc =
      module.lookupSymbol<mlir::func::FuncOp>("fold_value_demo");
  auto foldInPlaceFunc =
      module.lookupSymbol<mlir::func::FuncOp>("fold_inplace_demo");
  if (!foldFunc || !foldValueFunc || !foldInPlaceFunc)
    return module.emitError("expected demo func.funcs named @fold_demo, "
                            "@fold_value_demo, and @fold_inplace_demo");

  mlir::PassManager importPM(module.getContext());
  importPM.addPass(mlir::egraph::createConvertFuncToEGraphPass());
  if (mlir::failed(importPM.run(module)))
    return module.emitError("failed to import fold demos into egraph");

  auto constEGraph =
      module.lookupSymbol<mlir::egraph::EGraphOp>("fold_demo_egraph");
  auto valueEGraph =
      module.lookupSymbol<mlir::egraph::EGraphOp>("fold_value_demo_egraph");
  auto inPlaceEGraph =
      module.lookupSymbol<mlir::egraph::EGraphOp>("fold_inplace_demo_egraph");
  if (!constEGraph || !valueEGraph || !inPlaceEGraph)
    return module.emitError(
        "expected convert-func-to-egraph to create all fold demo egraphs");

  auto constAddi = constEGraph.lookupSymbol<mlir::egraph::EClassOp>("addi");
  auto valueAddi = valueEGraph.lookupSymbol<mlir::egraph::EClassOp>("addi");
  auto inPlaceOpB = inPlaceEGraph.lookupSymbol<mlir::egraph::EClassOp>("op_b");
  if (!constAddi || !valueAddi)
    return module.emitError("expected imported @addi roots in both fold demos");
  if (!inPlaceOpB)
    return module.emitError(
        "expected imported @op_b root in in-place fold demo");
  mlir::StringAttr constAddiName = constAddi.getSymNameAttr();
  mlir::StringAttr valueAddiName = valueAddi.getSymNameAttr();
  mlir::StringAttr inPlaceOpBName = inPlaceOpB.getSymNameAttr();
  if (!constAddiName || !valueAddiName || !inPlaceOpBName)
    return module.emitError("expected fold demo addi roots to carry sym_name");

  auto valueInput = valueEGraph.lookupSymbol<mlir::egraph::InputOp>("arg0");
  if (!valueInput)
    return valueEGraph.emitOpError("expected imported @arg0 input");
  mlir::StringAttr valueInputName = valueInput.getSymNameAttr();
  if (!valueInputName)
    return valueInput.emitOpError("expected @arg0 to carry a sym_name attr");

  mlir::egraph::EGraph constGraph;
  if (mlir::failed(constGraph.indexEGraph(constEGraph)))
    return constEGraph.emitOpError("failed to index imported const fold demo");
  if (!constGraph.isClean())
    return constEGraph.emitOpError(
        "expected imported fold demo egraph to start clean");

  mlir::egraph::EGraphPatternSet patterns;
  mlir::FailureOr<mlir::egraph::EGraphRewriteDriverResult> disabledDriven =
      mlir::egraph::applyEGraphPatterns(constGraph, patterns, module);
  if (mlir::failed(disabledDriven))
    return module.emitError("disabled fold driver failed");
  if (disabledDriven->changed || disabledDriven->changedCommits != 0 ||
      disabledDriven->rebuilds != 0)
    return module.emitError(
        "disabled fold driver unexpectedly changed the fold demo");

  module.emitRemark() << "egraph fold stayed disabled by default in @"
                      << constAddiName.getValue();

  mlir::egraph::EGraphRewriteDriverConfig config;
  config.enableMlirFold = true;

  unsigned conflictingPatternMatches = 0;
  patterns.add<FoldConflictAddToAliasPattern>(conflictingPatternMatches);

  mlir::FailureOr<mlir::egraph::EGraphRewriteDriverResult> driven =
      mlir::egraph::applyEGraphPatterns(constGraph, patterns, module, config);
  if (mlir::failed(driven))
    return module.emitError("enabled fold driver failed");
  if (!constGraph.isClean())
    return module.emitError("enabled fold driver left the graph dirty");
  if (!driven->changed || driven->changedCommits == 0)
    return module.emitError("enabled fold driver did not change the fold demo");
  if (conflictingPatternMatches != 0)
    return module.emitError(
        "enabled fold driver should skip user patterns after folding a root");
  if (!constGraph.getValue(constAddiName).hasDef<mlir::arith::AddIOp>())
    return constEGraph.emitOpError(
        "expected the original addi candidate to remain");
  if (!hasConstantIntegerDef(constGraph.getValue(constAddiName), 5))
    return constEGraph.emitOpError("expected a delayed folded constant 5");

  module.emitRemark()
      << "egraph fold preserved the original addi candidate in @"
      << constAddiName.getValue();
  module.emitRemark() << "egraph fold materialized delayed constant in @"
                      << constAddiName.getValue();
  module.emitRemark() << "egraph fold skipped conflicting user rewrite in @"
                      << constAddiName.getValue();

  mlir::egraph::EGraph valueGraph;
  if (mlir::failed(valueGraph.indexEGraph(valueEGraph)))
    return valueEGraph.emitOpError("failed to index imported value fold demo");
  if (!valueGraph.isClean())
    return valueEGraph.emitOpError(
        "expected imported value fold demo egraph to start clean");

  mlir::egraph::EGraphPatternSet valuePatterns;
  mlir::FailureOr<mlir::egraph::EGraphRewriteDriverResult> valueDriven =
      mlir::egraph::applyEGraphPatterns(valueGraph, valuePatterns, module,
                                        config);
  if (mlir::failed(valueDriven))
    return module.emitError("value fold driver failed");
  if (!valueGraph.isClean())
    return module.emitError("value fold driver left the graph dirty");
  if (!valueDriven->changed || valueDriven->changedCommits == 0)
    return module.emitError("value fold driver did not change the demo");
  if (!valueGraph.getValue(valueAddiName).hasDef<mlir::arith::AddIOp>())
    return valueEGraph.emitOpError(
        "expected the original addi candidate to remain");
  if (!valueGraph.getValue(valueAddiName)
           .isEquivalentTo(valueGraph.getValue(valueInputName)))
    return valueEGraph.emitOpError("expected delayed fold to alias @arg0");

  module.emitRemark()
      << "egraph fold preserved the original addi candidate in @"
      << valueAddiName.getValue();
  module.emitRemark() << "egraph fold materialized delayed value alias in @"
                      << valueAddiName.getValue();

  mlir::egraph::EGraph inPlaceGraph;
  if (mlir::failed(inPlaceGraph.indexEGraph(inPlaceEGraph)))
    return inPlaceEGraph.emitOpError(
        "failed to index imported in-place fold demo");
  if (!inPlaceGraph.isClean())
    return inPlaceEGraph.emitOpError(
        "expected imported in-place fold demo egraph to start clean");

  mlir::egraph::EGraphPatternSet inPlacePatterns;
  mlir::FailureOr<mlir::egraph::EGraphRewriteDriverResult> inPlaceDriven =
      mlir::egraph::applyEGraphPatterns(inPlaceGraph, inPlacePatterns, module,
                                        config);
  if (mlir::failed(inPlaceDriven))
    return module.emitError("in-place fold driver failed");
  if (!inPlaceGraph.isClean())
    return module.emitError("in-place fold driver left the graph dirty");
  if (!inPlaceDriven->changed || inPlaceDriven->changedCommits == 0)
    return module.emitError("in-place fold driver did not change the demo");

  bool sawTaggedOpB = false;
  bool sawTaglessOpB = false;
  for (mlir::egraph::EOpRef<mlir::egraph::test::OpBOp> ref :
       inPlaceGraph.getValue(inPlaceOpBName)
           .getDefs<mlir::egraph::test::OpBOp>()) {
    if (ref.getOp().getTag())
      sawTaggedOpB = true;
    else
      sawTaglessOpB = true;
  }
  if (!sawTaggedOpB || !sawTaglessOpB)
    return inPlaceEGraph.emitOpError(
        "expected both tagged and tagless op_b candidates after in-place fold");

  module.emitRemark()
      << "egraph fold preserved the original tagged op_b candidate in @"
      << inPlaceOpBName.getValue();
  module.emitRemark() << "egraph fold materialized an in-place tagless op_b "
                         "candidate in @"
                      << inPlaceOpBName.getValue();
  return mlir::success();
}

struct TestEGraphPipelinePass
    : public mlir::PassWrapper<TestEGraphPipelinePass,
                               mlir::OperationPass<mlir::ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TestEGraphPipelinePass)

  llvm::StringRef getArgument() const final { return "test-egraph-pipeline"; }

  llvm::StringRef getDescription() const final {
    return "test MLIR-EGraph arith demo import and rewrite pipeline";
  }
  ...
  void runOnOperation() final {
    if (mlir::failed(runImportedPipeline(getOperation())))
      signalPassFailure();
  }
};

struct TestEGraphMlirFoldPass
    : public mlir::PassWrapper<TestEGraphMlirFoldPass,
                               mlir::OperationPass<mlir::ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TestEGraphMlirFoldPass)

  llvm::StringRef getArgument() const final { return "test-egraph-mlir-fold"; }

  llvm::StringRef getDescription() const final {
    return "test delayed MLIR fold integration in the EGraph driver";
  }
  ...
  void runOnOperation() final {
    if (mlir::failed(runImportedFoldPipeline(getOperation())))
      signalPassFailure();
  }
};

struct TestEGraphExtractRequestPass
    : public mlir::PassWrapper<TestEGraphExtractRequestPass,
                               mlir::OperationPass<mlir::ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TestEGraphExtractRequestPass)

  llvm::StringRef getArgument() const final {
    return "test-egraph-extract-request";
  }

  llvm::StringRef getDescription() const final {
    return "test normalized graph extract request selection";
  }
  ...
  void runOnOperation() final {
    ...
    if (mlir::succeeded(result))
      result = verifyExtractRequestSemantics(module, graph);
...
void registerEGraphTestPasses() {
  mlir::PassRegistration<TestEGraphPipelinePass>();
  mlir::PassRegistration<TestEGraphMlirFoldPass>();
  mlir::PassRegistration<TestEGraphExtractRequestPass>();
